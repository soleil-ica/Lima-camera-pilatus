        ...
        switch(msg.type())
        {
            ...
            case yat::TASK_PERIODIC:
            {
                ...
                if(m_is_reader_watcher) // need to read *.tif files in order to display the frames
                {
                    ...
                    std::string full_file_pattern = m_cam.imgpath() + "/" + m_cam.fileName();
					std::string full_file_name = yat::String::str_format(full_file_pattern.c_str() , m_image_number);
										
					//- check if file exist					
					ssmsg.str("");
					ssmsg<<"Check if file : [" << full_file_name << "] exist ?"<<std::endl;
                    DEB_TRACE() << ssmsg.str(); 
