        ...
        switch(msg.type())
        {
            ...
            case yat::TASK_PERIODIC:
            {
                ...
                if(m_is_reader_watcher) // need to read *.tif files in order to display the frames
                {
					...
					ssmsg.str("");
					ssmsg<<"Force refreshing of nfs file system using ls command on : "<<m_cam.imgpath()<<std::endl;
                    DEB_TRACE() << ssmsg.str();
                    // Force nfs file system to refresh !!
                    std::stringstream ls_command;
                    ls_command << "ls " << m_cam.imgpath() << " 2>&1 > /dev/null";
                    system(ls_command.str().c_str());

					ssmsg.str("");
					ssmsg<<"Check if file : [" << full_file_name.c_str() << "] exist ?"<<std::endl;
                    DEB_TRACE() << ssmsg.str(); 
                    //- check if file exist
                    std::ifstream input_file(full_file_name.c_str(), ios_base::in);
                    if (input_file)
                    {
                        //foreach file generated by CamServer & found by Reader, a new frame is added
						ssmsg.str("");
						ssmsg<<"-- Found File [" << full_file_name.c_str() << "]"<<std::endl;
                        DEB_TRACE() << ssmsg.str(); 
						ssmsg.str("");
						ssmsg<<"-- Add New Frame ..."<<std::endl;
						DEB_TRACE() << ssmsg.str(); 						
            ...
            case PILATUS_STOP_MSG:
            {
                ...
                {
					std::stringstream ssmsg("");
                    Camera::Status cam_status = Camera::KILL_ACQUISITION;
                    do
                    {
                        DEB_TRACE()<<"wait KILL_ACQUISITION ...";
                        yat::ThreadingUtilities::sleep(0, 100000000); //100 ms
